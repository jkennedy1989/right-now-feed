...
export function ListViewMode() {
  ...
  // Scroll to active card when pin is clicked (activeIndex changes externally)
  useEffect(() => {
    if (!scrollRef.current || !listViewMode) return;
    const cardWidth = scrollRef.current.offsetWidth;
    const targetScroll = listViewMode.activeIndex * cardWidth;
    if (Math.abs(scrollRef.current.scrollLeft - targetScroll) > 10) {
      isScrollingProgrammatically.current = true;
      scrollRef.current.scrollTo({ left: targetScroll, behavior: 'smooth' });
      setTimeout(() => { isScrollingProgrammatically.current = false; }, 400);
    }
  }, [listViewMode?.activeIndex]);
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* Header */}
      <div className="flex items-center justify-between bg-white/90 backdrop-blur-xl rounded-full px-4 py-2 shadow-lg border border-white/30 mx-4">
      ...
      {/* Card carousel */}
      {isLoading ? (
        <div className="bg-white rounded-2xl shadow-lg p-4 mx-4">
